...
namespace {
  ...
  Value search(Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode) {
    ...
    if (   !PvNode
        ...
        &&  eval >= beta
        &&  eval >= ss->staticEval
        &&  ss->staticEval >= beta - 15 * depth - improvement / 15 + 201 + ss->complexity / 24
        && !excludedMove
        ...
        && (ss->ply >= thisThread->nmpMinPly || us != thisThread->nmpColor))
    {
        ...
        Depth R = std::min(int(eval - beta) / 147, 5) + depth / 3 + 4 - (ss->complexity > 650);
    ...
    if (    PvNode
        && !ttMove)
        depth -= 3   + (ss->complexity / 588) - ((ss-1)->complexity / 563)
                     + ((ss-2)->complexity / 654) - ((ss-3)->complexity / 649)
                     + ((ss-4)->complexity / 545) - ((ss-5)->complexity / 596)
                     + ((ss-6)->complexity / 653) - ((ss-7)->complexity / 522);
