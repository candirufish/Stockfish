...
moves_loop: // When in check, search starts here
    ...
    while ((move = mp.next_move(moveCountPruning)) != MOVE_NONE)
    {
      ...
      if (improving && !ss->inCheck && !(ss-2)->inCheck && !(ss-4)->inCheck)
      {
      int range = std::max({(ss-4)->staticEval, (ss-2)->staticEval, ss->staticEval}) - 
          std::min({(ss-4)->staticEval, (ss-2)->staticEval, ss->staticEval});
      if (range > 32)
          r--;
      }
