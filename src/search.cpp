...
namespace {
  ...
  Value search(Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode) {
    ...
    improvement =   (ss-2)->staticEval != VALUE_NONE ? ss->staticEval - ((ss-2)->staticEval + 16 * (complexity < 16))
                  : (ss-4)->staticEval != VALUE_NONE ? ss->staticEval - ((ss-4)->staticEval + 16 * (complexity < 16))
                  :                                    172;
