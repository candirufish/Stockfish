...
namespace {
  ...
  constexpr int KingAttackWeights[PIECE_TYPE_NB] = { 0, 0, 77, 55, 44, 10 };
   ...
   int KingDangerMult[6] = {64, 182, 128, 857, 8, 31};
...
#define S(mg, eg) make_score(mg, eg)
  ...
  Score ThreatByKing[] = { S(25, 57), S(4, 139) };
  ...
  Score KingProtector[] = { S(3, 5), S(5, 3), S(3, 0), S(0, -2) };

  // Assorted bonuses and penalties
  constexpr Score BishopPawns        = S(  3,  5);
  Score CloseEnemies       = S(  8,  0);
  constexpr Score Connectivity       = S(  3,  1);
  constexpr Score CorneredBishop     = S( 50, 50);
  constexpr Score Hanging            = S( 52, 30);
  Score HinderPassedPawn   = S(  5,  2);
  ...
  Score ThreatByPawnPush   = S( 49, 30);
  constexpr Score ThreatByRank       = S( 16,  3);
  Score ThreatBySafePawn   = S(186,140);
  constexpr Score TrappedRook        = S( 92,  0);
  constexpr Score WeakQueen          = S( 50, 10);
  Score WeakUnopposedPawn  = S(  14, 19);
  
   TUNE(SetRange(a_range),ThreatByKing,ThreatBySafePawn);
   TUNE(SetRange(b_range),PassedFile,ThreatByPawnPush,WeakUnopposedPawn,KingDangerMult);
